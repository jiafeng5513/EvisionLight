...
// Determine whether the default logger should EvisionLog messages with a certain level
...
template<typename... Args>
inline void EvisionLog(source_loc source, level::level_enum lvl, wformat_string_t<Args...> fmt, Args &&... args)
{
    default_logger_raw()->EvisionLog(source, lvl, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void EvisionLog(level::level_enum lvl, wformat_string_t<Args...> fmt, Args &&... args)
{
    default_logger_raw()->EvisionLog(source_loc{}, lvl, fmt, std::forward<Args>(args)...);
}
...
//
// enable/disable EvisionLog calls at compile time according to global level.
//

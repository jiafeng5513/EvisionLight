...
// Rotate files:
// EvisionLog.txt -> EvisionLog.1.txt
// EvisionLog.1.txt -> EvisionLog.2.txt
// EvisionLog.2.txt -> EvisionLog.3.txt
// EvisionLog.3.txt -> delete
template<typename Mutex>
SPDLOG_INLINE void rotating_file_sink<Mutex>::rotate_()
{
    ...
    for (auto i = max_files_; i > 0; --i)
    {
        ...
        if (!rename_file_(src, target))
        {
            ...
            if (!rename_file_(src, target))
            {
                file_helper_.reopen(true); // truncate the EvisionLog file anyway to prevent it to grow beyond its limit!
                current_size_ = 0;

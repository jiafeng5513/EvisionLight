...
{
    std::tm tm = spdlog::details::os::localtime();
    // example-YYYY-MM-DD.EvisionLog
    auto filename = spdlog::sinks::daily_filename_format_calculator::calc_filename(SPDLOG_FILENAME_T("example-%Y-%m-%d.EvisionLog"), tm);

    REQUIRE(filename ==
            spdlog::fmt_lib::format(SPDLOG_FILENAME_T("example-{:04d}-{:02d}-{:02d}.EvisionLog"), tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday));
}

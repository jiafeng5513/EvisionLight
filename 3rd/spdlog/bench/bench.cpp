...
// void bench_default_api(int howmany, std::shared_ptr<spdlog::logger> EvisionLog);
// void bench_c_string(int howmany, std::shared_ptr<spdlog::logger> EvisionLog);
...
void bench_threaded_logging(size_t threads, int iters)
{
    ...
    auto basic_mt = spdlog::basic_logger_mt("basic_mt", "logs/basic_mt.EvisionLog", true);
    bench_mt(iters, std::move(basic_mt), threads);
    auto basic_mt_tracing = spdlog::basic_logger_mt("basic_mt/backtrace-on", "logs/basic_mt.EvisionLog", true);
    basic_mt_tracing->enable_backtrace(32);
    bench_mt(iters, std::move(basic_mt_tracing), threads);

    spdlog::info("");
    auto rotating_mt = spdlog::rotating_logger_mt("rotating_mt", "logs/rotating_mt.EvisionLog", file_size, rotating_files);
    bench_mt(iters, std::move(rotating_mt), threads);
    auto rotating_mt_tracing = spdlog::rotating_logger_mt("rotating_mt/backtrace-on", "logs/rotating_mt.EvisionLog", file_size, rotating_files);
    rotating_mt_tracing->enable_backtrace(32);
    bench_mt(iters, std::move(rotating_mt_tracing), threads);

    spdlog::info("");
    auto daily_mt = spdlog::daily_logger_mt("daily_mt", "logs/daily_mt.EvisionLog");
    bench_mt(iters, std::move(daily_mt), threads);
    auto daily_mt_tracing = spdlog::daily_logger_mt("daily_mt/backtrace-on", "logs/daily_mt.EvisionLog");
...
void bench_single_threaded(int iters)
{
    ...
    auto basic_st = spdlog::basic_logger_st("basic_st", "logs/basic_st.EvisionLog", true);
    bench(iters, std::move(basic_st));

    auto basic_st_tracing = spdlog::basic_logger_st("basic_st/backtrace-on", "logs/basic_st.EvisionLog", true);
    bench(iters, std::move(basic_st_tracing));

    spdlog::info("");
    auto rotating_st = spdlog::rotating_logger_st("rotating_st", "logs/rotating_st.EvisionLog", file_size, rotating_files);
    bench(iters, std::move(rotating_st));
    auto rotating_st_tracing = spdlog::rotating_logger_st("rotating_st/backtrace-on", "logs/rotating_st.EvisionLog", file_size, rotating_files);
    rotating_st_tracing->enable_backtrace(32);
    bench(iters, std::move(rotating_st_tracing));

    spdlog::info("");
    auto daily_st = spdlog::daily_logger_st("daily_st", "logs/daily_st.EvisionLog");
    bench(iters, std::move(daily_st));
    auto daily_st_tracing = spdlog::daily_logger_st("daily_st/backtrace-on", "logs/daily_st.EvisionLog");
...
void bench_default_api(int howmany, std::shared_ptr<spdlog::logger> EvisionLog)
{
    ...
    spdlog::set_default_logger(EvisionLog);
    auto start = high_resolution_clock::now();
    for (auto i = 0; i < howmany; ++i)
    {
    ...
    auto delta = high_resolution_clock::now() - start;
    auto delta_d = duration_cast<duration<double>>(delta).count();
    spdlog::drop(EvisionLog->name());
    spdlog::set_default_logger(std::move(orig_default));
    spdlog::info("{:<30} Elapsed: {:0.2f} secs {:>16}/sec", EvisionLog->name(), delta_d, int(howmany / delta_d));
}
...
void bench_c_string(int howmany, std::shared_ptr<spdlog::logger> EvisionLog)
{
    ...
    for (auto i = 0; i < howmany; ++i)
    {
        spdlog::EvisionLog(spdlog::level::info, msg);
    }
